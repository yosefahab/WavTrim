...
// followed by a sequence of data chunks. A WAVE file is often just a RIFF file
...
typedef struct WAV_HEADER {
  ...
  uint32_t ChunkSize; // RIFF Chunk size in bytes, the entire file in bytes
                      // minus 8 bytes (ChunkID and ChunkSize)
  ...
  uint32_t Subchunk1Size; // fmt chunk size in bytes, minus 8 bytes (Subchunk1ID
                          // and Subchunk1Size)
...
constexpr bool sanity_check_header(const Wav_hdr &wavHeader) {
  return ((wavHeader.SampleRate * wavHeader.NumChannels *
           wavHeader.BitsPerSample / 8) == wavHeader.ByteRate) &&
...
inline int8_t *read_data(FILE *wavFile, Wav_hdr &wavHeader,
                         const float &trimRatio, const bool &fromEnd) {
  ...
  const size_t bytesRead =
      fread(buffer, sizeof(buffer[0]),
            wavHeader.Subchunk2Size / sizeof(buffer[0]), wavFile);
...
inline void save_wav(const string &fileName, const Wav_hdr &wavHeader,
                     int8_t *data) {
